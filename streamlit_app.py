...
# --- Secrets format (matches user's existing config) ---
# [[cms]]
# name = "RTS"
...
# password = "******"
#
# Optional (if you don't upload at runtime):
# bom_index_path = "data/BOM_Index.csv"

def get_instances_from_secrets():
    cms_list = st.secrets.get("cms", [])
    # cms_list is expected to be a list of dicts with keys: name, url, username, password
    if isinstance(cms_list, dict):
        # In case someone put a single table instead of an array; normalize
        cms_list = [cms_list]
...
if not instances:
    st.error("No CMS instances configured in secrets (expected [[cms]] array).")
    st.stop()
...
endpoint_hint = st.text_input("Optional controllers endpoint override", value="/rest/display-controller/list")
...
if btn:
    ...
    if not all([base_url, username, password]):
        st.error("Missing url/username/password for the selected instance in secrets.")
        st.stop()
    ...
    with st.status("Fetching display controllers...", expanded=False) as s2:
        try:
            controllers = fetch_controllers(sess, base_url, endpoint_hint)
